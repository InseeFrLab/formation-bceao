...
- Deux formats 
  - `Parquet` : stockage disque
  - `Arrow` : traitement en mémoire

## Modes de stockage

- Le [**stockage hybride**]{.orange} optimise la lecture
  - Projection (`SELECT`) : orientation colonne
  - Predicates (`WHERE`) : orientation ligne
...
- `Arrow` ne résout pas tout
  - Les données massives nécessitent des infrastuctures big data
...
- Insee : deux [**labs**]{.orange} qui travaillent en synérgie
  - [**SSP Lab**]{.orange} : innovation méthodologique en data science
  - [**DIIT**]{.orange} : innovation informatique (Onyxia / SSP Cloud)
...
- Toutes nos [formations](https://www.sspcloud.fr/formation) sont [**open-source**]{.blue2}
